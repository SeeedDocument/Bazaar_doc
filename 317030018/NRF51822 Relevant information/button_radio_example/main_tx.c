...
void init(void)
{
  ...
  // Turn peripherals on
  // Power on the PPI peripheral
  NRF_POWER->PERPOWER |= (POWER_PERPOWER_PPI_Power << POWER_PERPOWER_PPI_Pos);

  // Wait for the power up
  while ((NRF_POWER->PERRDY & POWER_PERRDY_PPI_Msk) != (POWER_PERRDY_PPI_Ready << POWER_PERRDY_PPI_Pos))
  {
  }

  // Power on the Radio
  NRF_POWER->PERPOWER |= (POWER_PERPOWER_RADIO_Power << POWER_PERPOWER_RADIO_Pos);

  // Wait for power up
  while ((NRF_POWER->PERRDY & POWER_PERRDY_RADIO_Msk) != (POWER_PERRDY_RADIO_Ready << POWER_PERRDY_RADIO_Pos))
  {
  }
